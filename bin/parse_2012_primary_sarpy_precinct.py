...
url = 'http://results.enr.clarityelections.com/NE/Sarpy/39061/83203/en/reports.html'
...
def clarify_sarpy():
    ...
    filename = '../' + p.election_date.strftime('%Y') + '/' + '__'.join([
        election_date,
        'ne',
        election_type,
        county.lower(),
    ...
    with open(filename, 'wb') as outfile:
        ...
        for row in results:
            total_votes = row['Absentee'] + row['Provisional'] \
                          + row['Election Night'] + row['Early Pick-up']

            # According to the Sarpy County election commissioner,
            # "Early Pick-up" votes are just election-day ballots that
            # were collected and counted earlier than other ballots
            election_night = row['Election Night'] + row['Early Pick-up']

            f.writerow([row['county'], row['precinct'], row['office'],
                       row['district'], row['party'], row['candidate'],
                       total_votes, election_night,
